...
import { getAssets, createAsset, deleteAsset } from '../api/client';
...
export default function AssetsScreen() {
  ...
  const [form, setForm] = useState({ name: '', type: 'cash', value: '', description: '' });
  ...
  const handleAdd = async () => {
    ...
    setSaving(true);
    try {
      await createAsset({ ...form, value: parseFloat(form.value) });
      setModalVisible(false);
      setForm({ name: '', type: 'cash', value: '', description: '' });
      load();
    } catch (err) {
      Alert.alert('Error', err.response?.data?.error || 'Could not create asset');
    } finally {
      setSaving(false);
    }
  };
      ...
      <Modal visible={modalVisible} animationType="slide" presentationStyle="pageSheet">
        <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
          <ScrollView style={styles.modal} contentContainerStyle={styles.modalContent}>
            ...
            <TextInput style={styles.input} value={form.name} onChangeText={(v) => setForm({ ...form, name: v })} placeholder="e.g. Savings Account" placeholderTextColor="#9ca3af" />
            ...
            <TextInput style={styles.input} value={form.value} onChangeText={(v) => setForm({ ...form, value: v })} placeholder="0.00" placeholderTextColor="#9ca3af" keyboardType="decimal-pad" />
            ...
            <TouchableOpacity style={styles.saveBtn} onPress={handleAdd} disabled={saving}>
              {saving ? <ActivityIndicator color="#fff" /> : <Text style={styles.saveBtnText}>Add Asset</Text>}
            </TouchableOpacity>
          </ScrollView>
